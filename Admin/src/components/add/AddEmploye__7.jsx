    ...
    useEffect(() => {
        if (localStorage.getItem('mat') !== null) {
            axios.get(`${process.env.REACT_APP_API}/employe/${localStorage.getItem('mat')}`).then((response) => {
                ...
                // setValue("absence_irr", '')
                // setValue("date", todayUpdateFunction(tab['annee']))
                // setValue("date_fin", todayUpdateFunction(tab['date_fin']))
                // setValue("conge", '')
            }).catch((err) => {
                console.log(err)
            })
                ...
                <section className="coll-2 " ref={coll__2}>
                    ...
                    <div className="coll-2-container" >
                        ...
                        <form onSubmit={handleSubmit(submitForm)} style={{ margin: '0 auto' }}>
                            ...
                            <div className="user-input-wrp" >
                                <br />
                                <select className="inputText"  {...register("conge")} style={{ paddingTop: '14px', paddingBottom: '14px', height: '50px' }} id="selRegion" required >
